...
《怪物猎人：荒野》的游戏内音频捕获与替换插件。需要制作语音包时，可以开启实时采集来辨认游戏音频；使用已有语音包时，分组规则仍会匹配并替换游戏声音。外部音频支持 MP3、WAV、OGG 格式。
...
flowchart TD
    ...
    E -- 否 --> CAP{近期事件实时采集已开启?}
    CAP -- 是 --> G[分类并聚合到近期事件]
    CAP -- 是 --> AC[快照来源角色当前主动作]
    CAP -- 否 --> RA[仅规则需要时读取主动作]
    RA --> J{命中已启用规则?}
    CAP -- 是 --> J
    AC --> AK[记录动作键]
    AK --> G
...
普通语音包使用者无需开启“近期事件”的实时采集。该开关默认关闭；关闭时近期列表不收集其他音效，分组规则、动作绑定候选、外部音频播放和原声抑制仍会运行。安装 REFF 后，需要捕获新事件时再手动开启。
...
3. 打开“实时采集”开关。该开关默认关闭，Script Reset 或重启游戏后也会恢复为关闭。
4. 在游戏中自然触发想要替换的声音。
5. 必要时输入来源名称并点击锁定。锁定后只有同时满足当前分类和搜索条件的事件会进入记录列表。
6. 点击播放确认目标事件，再点击收藏。

关闭实时采集后，近期列表立即清空，其他音频不再进入列表或累计触发次数；再次开启时从空列表重新记录。关闭期间发生的事件不会补录，也不会给保存列表追加新的已记录动作。分组规则仍按稳定键匹配，绑定动作的候选会按需读取主动作并继续替换。
...
要为保存列表继续记录新动作，请先开启“近期事件”的实时采集；关闭时已有记录保持不变。
...
#### v1.1.1: 性能优化
1. 近期事件新增按需开启的实时采集开关，默认关闭；切换开关时清空列表并重置触发次数，REF的Script Reset 或重启游戏后恢复为关闭。
2. 关闭采集时跳过普通音频的近期记录与游戏内重放解析；分组替换、动作绑定候选和原声抑制保持运行。

#### v1.1.0: 
1. 为保持的音频增加动作检测，可对使用相同音频的不同动作绑定不同的候选音频。
2. 优化REFAudio的log输出。
