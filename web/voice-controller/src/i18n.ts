...
const messages = {
  'zh-CN': {
    ...
    action: {all: '所有动作', controller: '控制器', category: '分类', id: '动作'},
    status: {
    ...
    saved: {
      ...
      observedActions: '已记录动作：', actionUnavailable: '尚未解析到动作',
    ...
    groups: {
      ...
      conflicts: '冲突明细', actionConflict: '动作 {key}', active: '生效中', noActive: '无生效规则', allDisabled: '该音频的所有规则当前都被禁用', occupied: '被占用',
      ...
      selectSaved: '从保存列表选择游戏内音频', selectAction: '选择触发动作（可选）', selectAudio: '选择该分组的外部音频', emptyAudio: '该分组 Audio 文件夹为空',
  ...
  'en-US': {
    ...
    action: {all: 'All actions', controller: 'Controller', category: 'Category', id: 'Action'},
    status: {
    ...
    saved: {
      ...
      observedActions: 'Recorded actions:', actionUnavailable: 'No action resolved yet',
    ...
    groups: {
      ...
      conflicts: 'Conflict Details', actionConflict: 'Action {key}', active: 'Active', noActive: 'No active rule', allDisabled: 'All rules for this audio are currently disabled', occupied: 'Occupied',
      ...
      selectSaved: 'Select in-game audio from saved audio', selectAction: 'Select triggering action (optional)', selectAudio: 'Select external audio for this group', emptyAudio: 'This group Audio folder is empty',
